...
function Controller() {
    ...
    function getDynamicLeftPos(oppCount) {
        ...
        oppCount > 9 && 99 >= oppCount ? dynamicLeftPos = 85 : oppCount > 99 && 999 >= oppCount && (dynamicLeftPos = 95);
    ...
    function constructChallengeRows(obj, index, type) {
        ...
        if (true != child) {
            ...
            350 > Titanium.Platform.displayCaps.platformWidth && (rightPercentage = "3%");
            row.add(Ti.UI.createLabel({
                font: {
                    fontFamily: font
                },
        ...
        xhr.onload = function() {
            if ("200" == this.status) {
                if (4 == this.readyState) {
                    ...
                    for (child in $.challengesView.children) $.challengesView.children[child] = null;
