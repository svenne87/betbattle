...
function Controller() {
    ...
    function createAndShowTableView(league, array) {
        ...
        refresher.addEventListener("refreshstart", function() {
            if (Alloy.Globals.checkConnection()) getGames(leagueId, true, 0, 20); else {
            ...
            xhr.onload = function() {
                if ("200" == this.status) {
                    if (4 == this.readyState) {
                        ...
                        numberOfGamesFetched = numberOfGamesFetched >= totalNumberOfGames ? totalNumberOfGames : start;
                        if (array.length > 0) if (firstTime) {
                            createAndShowTableView(league, array);
                            numberOfGamesFetched = 20;
                            setDisplayText();
                        } else {
                            appendToRow(array);
                            numberOfGamesFetched - 0 + 20 >= totalNumberOfGames && (numberOfGamesFetched = totalNumberOfGames);
                            setDisplayText();
