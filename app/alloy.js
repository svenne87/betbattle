...
Alloy.Globals.focusWindow = function() { return true; };
...
if ( typeof lang == 'undefined' || lang == '' || lang == null) {
	var localeString = Titanium.Locale.getCurrentLanguage().toLowerCase();
	
	if(localeString.length > 2) {
		localeString = localeString.substring(0, 2);
	}
	
    Alloy.Globals.LOCALE = localeString;  
} else {
    lang = lang.language;
...
Alloy.Globals.BETKAMPENGETFRIENDSANDGROUPSURL = Alloy.Globals.BETKAMPENURL + '/api/get_friends_and_groups.php';
...
Alloy.Globals.BETKAMPENFRIENDSEARCHURL = Alloy.Globals.BETKAMPENURL + '/api/get_user_group_search.php';
